...
import { developerProfileTool, coffeeKnowledgeTool } from './tools/5f8a6d84-81c9-42a5-b99f-2fb5b666e9cd.mjs';
...
const coffeeSommelierAgent = new Agent({
  name: "Coffee Sommelier for Developers",
  description: "A witty, expert coffee sommelier that matches coffee to developer personalities and coding preferences",
  instructions: `
...
z.object({
  ...
  reasoning: z.string().describe("Witty explanation connecting the coffee to their coding preferences")
});
...
const generateRecommendationStep = createStep({
  ...
  execute: async ({ inputData, getInitData }) => {
    ...
    `;
    try {
      const response = await coffeeSommelierAgent.generate([
...
const coffeeRecommendationWorkflow = createWorkflow({
  id: "coffee-recommendation",
  description: "Generate personalized coffee recommendations for developers based on their coding preferences",
  inputSchema: z.object({
    language: z.enum(["Node.js", "Python", "Java", "Go", "Ruby", ".NET", "PHP"]),
    framework: z.string(),
    ide: z.string(),
...
}).then(prepareKnowledgeQueryStep).then(gatherKnowledgeStep).then(generateRecommendationStep).commit();

const mastra = new Mastra({
  agents: {
    coffeeSommelierAgent
  },
  workflows: {
    coffeeRecommendationWorkflow
  },
